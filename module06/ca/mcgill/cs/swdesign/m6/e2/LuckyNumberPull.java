 ...
 * 
 */
package ca.mcgill.cs.swdesign.m6.e2;
...
/**
 * @author Halil Murat
 * An equivalent of LuckyNumber class but 
 * in this one data-flows between the model and the observers are using the pull strategy.
 */
public class LuckyNumberPull extends Application
{
...
class SliderPanel extends HBox implements Observer
{
	...
	{
		// Model no longer gives the new number so the observer has to get it itself
		aSlider.setValue(new Integer(aModel.getNumber()));
	}
	...
	private static Slider createSlider()
	{
		...
		slider.setMinWidth(LuckyNumberPull.WIDTH);
...
class IntegerPanel extends HBox implements Observer
{
	...
	IntegerPanel(Model pModel)
	{
		aModel = pModel;
		aModel.addObserver(this);
		aText.setMinWidth(LuckyNumberPull.WIDTH); 
...
interface Observer
{
	// In pull strategy the callback method no longer has a parameter
	void newNumber();
}